...
class Test : DescribeSpec() {
    ...
    init {
        describe("Тестирование поиска даты для установки напоминания") {
            it("(1)У контакта в этом году прошёл день рождение, дата уведомления должна быть в следущем году") {
                ...
                val birthdayOfContactCalendar
                = Calendar.getInstance()
                birthdayOfContactCalendar
                    .set(1996, 8, 8)
                val notidicationCalendarFromMethod = putNextBirthday(birthdayOfContactCalendar
                    , calendarNow)
            ...
            it("(3)День рождения контакта в високосном году 29 февраля,дата напоминания должна быть в следущем году т.к." +
            ...
            it("(4)День рождения контакта 29 февраля, в этом году был день рождения, " +
    ...
    private fun checkOfMethod(notidicationCalendar: Calendar, notidicationCalendarFromMethod: Calendar) {
        notidicationCalendarFromMethod.get(Calendar.DAY_OF_MONTH) shouldBe notidicationCalendar.get(Calendar.DAY_OF_MONTH)
        notidicationCalendarFromMethod.get(Calendar.MONTH) shouldBe notidicationCalendar.get(Calendar.MONTH)
